...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(file, line)) {
        ...
        leda::integer x1n, x1d, y1n, y1d, x2n, x2d, y2n, y2d;

        // Read and convert the first token (x1n, x1d)
        std::getline(ss, token, ';');
        x1n = leda::integer(token);  // Convert string to leda::integer
        std::getline(ss, token, ';');
        x1d = leda::integer(token);

        // Read and convert the second token (y1n, y1d)
        std::getline(ss, token, ';');
        y1n = leda::integer(token);
        std::getline(ss, token, ';');
        y1d = leda::integer(token);

        // Read and convert the third token (x2n, x2d)
        std::getline(ss, token, ';');
        x2n = leda::integer(token);
        std::getline(ss, token, ';');
        x2d = leda::integer(token);

        // Read and convert the fourth token (y2n, y2d)
        std::getline(ss, token, ';');
        y2n = leda::integer(token);
        std::getline(ss, token, ';');
        y2d = leda::integer(token);

        segments.push_back(leda::rat_segment(leda::rat_point(leda::rational(x1n, x1d), leda::rational(y1n, y1d)), leda::rat_point(leda::rational(x2n, x2d), leda::rational(y2n, y2d))));
    }
    file.close();
    ...
    if (print_intersections) {
        ...
        for (const leda::rat_point& p : intersection_points) {
            std::cout << p.xcoord().numerator() << ";" << p.xcoord().denominator();// Print m_x as binary
            std::cout << ";"; // Separate with a semicolon
            std::cout << p.ycoord().numerator() << ";" << p.ycoord().denominator(); // Print m_y as binary
