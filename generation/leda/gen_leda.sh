...
for d in {1..10}; do
	...
	for ((k = 10; k <= 60; k += 10)); do # Originally <=100 but this would yield numbers with more than 64 bits.
		...
		segintbench-convert float2bin ${TEMP_DIR}/leda_sweep_time_difficult_d${d}_k${k}_integer.csv testsets-binary/leda_sweep_time_difficult_d${d}_k${k}_integer.csv
